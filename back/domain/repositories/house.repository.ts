...
export interface IHouseRepository {
	...
	getAllWithFavoritesOf(userId: number);
...
export class PrismaHouseRepository implements IHouseRepository {
	...
	async getAllWithFavoritesOf(userId: number) {
		const q = `SELECT h.*, CASE WHEN f."userId" IS NOT NULL THEN true ELSE false END as isFavorite FROM "House" h LEFT JOIN "Favorite" f ON h.id = f."houseId" AND f."userId" = ${userId}`;
		const res: any = await this.prisma.$queryRawUnsafe(q);
		return res.map(HouseMapper.fromPrismaQuery);
	}
